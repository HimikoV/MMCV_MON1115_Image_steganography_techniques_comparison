# Usage: py pvdEncode.py <text+file> <Cover Image>
# Eg:    py pvdEncode.py secret_message.txt test.png
# Log of changes: encoded.log
#
...
def embedbits(i, j, pixel, diff, colorpixel):
    ...
    # Initialise
    pad = 0
    # nb - amount of bits to be embedded
    nb = diff

    # If the number of bits required is less than the number of bits in the data(char.) to be Embedded
    if nb < len(bits):

        # Get ambount of bits equal to nb into newbits
        newbits = bits[:nb]
        # Delete nb bist out of bits
        bits = bits[nb:]
        # Value of pixel colour
        val = colorpixel
        # Data to be embedded
        data = newbits
        # Get binary representation of an int
        bival = bin(val)
        # Cut the '0b' header from binary representation
        bival = bival[2:]
        # Replace lsb with nb bits
